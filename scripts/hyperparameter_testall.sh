...
echo "=========================================="
echo "Hyperparameter Tuning - Full Grid Search"
echo "=========================================="
echo "Datasets: 10 biomedical datasets"
echo "Shots: ${SHOTS}"
echo "Learning rates: ${LEARNING_RATES[@]}"
echo "Weight decay: 0.0 and 0.02 (testing all combinations)"
echo "Total combinations: $((${#LEARNING_RATES[@]} * 2))"
echo "Seeds: 1 (for speed)"
echo ""
echo "Estimated time: 6-10 hours (full grid search)"
echo "=========================================="
echo ""
...
# Test all combinations of learning rate and weight decay
...
do
    ...
    do
        ...
        echo ""
        echo "=========================================="
        echo "Testing LR=${LR} with Weight Decay=${WD}"
        echo "=========================================="
        
        # Create modified config files for this combination
        ...
        echo "✓ Completed all datasets for LR=${LR}, WD=${WD}"
    done
done
...
# Read results for all combinations
learning_rates = ['0.001', '0.0045', '0.005', '0.006', '0.010']
weight_decays = ['0.0', '0.02']

for lr in learning_rates:
    for wd in weight_decays:
        output_dir = f'${OUTPUT_BASE}/lr_{lr}_wd_{wd}'
        ...
        if csv_files:
            ...
            if all_results:
                ...
                results[(float(lr), float(wd))] = avg_result
                if float(lr) == 0.005 and float(wd) == 0.0:
...
for (lr, wd) in sorted(results.keys(), key=lambda x: (x[0], x[1])):
    ...
    print(f'{lr:<15} {wd:<15} {acc:.2f}%{baseline_str:<15} {diff_str:<15}')
