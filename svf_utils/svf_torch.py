...
def d_nsvd(matrix, rank=1):
    """
    Compute SVD and split singular values, left and right singular vectors.

    Args:
        matrix: (m, n) torch.Tensor
        rank: int, number of top singular values

    Returns:
        U1: (m, rank) top left singular vectors
        U2: (m, m-rank) bottom left singular vectors
        S1: (rank,) top singular values
        S2: (min(m,n)-rank,) bottom singular values
        V1: (rank, n) top right singular vectors
        V2: (n-rank, n) bottom right singular vectors
    """
    ...
    V = V.T  # rows are right singular vectors
    V = V[:rank, :]
...
class SVD_Conv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
                 ...
                 padding_mode='zeros', device=None, dtype=torch.double,
                 rank=1):
...
class SVD_Linear(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias, device=None, dtype=torch.double, rank=1):
        ...
        self.fc_V = nn.Linear(in_features, self.rank, False)#.double()
        self.vector_S = nn.Parameter(torch.empty((1, self.rank), **factory_kwargs))#.to('cuda:2').double() #+ 1e-10
        self.fc_U = nn.Linear(self.rank, out_features, bias)#.double()
            ...
            if len(model._modules[sub_module_name]._modules) > 0:
                ...
                if type(getattr(model, sub_module_name)) in full2low_mapping and \
                    ...
                    if old_type == nn.Linear:
                        fc1 = new_module._modules["fc_V"]
                        fc2 = new_module._modules["fc_U"]
                        param_list = lowrank_param_dict[current_module_name]
                        fc1.weight.data.copy_(param_list[1])
                        fc2.weight.data.copy_(param_list[0])
                        full_rank = new_module._modules["fc_U"].weight.data.shape[1]
                        new_module.vector_S.data.copy_(param_list[2])
                        
                        if bias_tensor is not None:
                            fc2.bias.data.copy_(bias_tensor)
    return model


class DatafreeSVD(object):

    def __init__(self, model, global_rank_ratio=1.0, vision_low_rank=768, text_low_rank=512,
                 excluded_layers=[], customized_layer_rank_ratio={}, skip_1x1_linear = True, skip_1x1=True, skip_3x3=True):
        ...
        self.model_cpu = self.model.module if model_data_parallel else self.model
        self.model_named_modules = self.model_cpu.named_modules()
        self.rank_base = 4
        self.global_rank_ratio = global_rank_ratio
        self.vision_low_rank = vision_low_rank
        self.text_low_rank = text_low_rank
        ...
        self.skip_1x1_linear = skip_1x1_linear

        

        self.param_lowrank_decomp_dict = {}
        registered_param_op = [nn.Linear]
        ...
        for m_name, m in self.model_named_modules:
            if type(m) in registered_param_op and m_name not in self.excluded_layers:
                ...
                tensor_shape = weights_tensor.shape#.squeeze().shape
                param_1x1_linear = False
                param_1x1 = False
                param_3x3 = False
                depthwise_conv = False
                if len(tensor_shape) == 2:
                    full_rank = min(tensor_shape[0], tensor_shape[1])
                    param_1x1_linear = True
                ...
                if self.skip_1x1_linear and param_1x1_linear:
                    continue
...
def resolver(
    ...
    lowrank_resolver = DatafreeSVD(model,
                                   ...
                                   skip_1x1_linear=skip_1x1_linear,
                                   skip_1x1=skip_1x1,
                                   skip_3x3=skip_3x3)
